lib: {
  types = {
    ...
    is = name: value: value.__type__ or null == name;

    ## Assign a type to an attribute set.
    ##
    ## @type String -> Attrs -> Attrs
    set = name: value: value // { __type__ = name; };
    ...
    functor = name: {
      inherit name;
      type = lib.attrs.select (lib.strings.split "." name) null lib.types;
      wrapped = null;
      payload = null;
      merge = null;
    };
    ...
    merge =
      ...
      in
      if f.name != g.name then
        null
      else if f.wrapped == null && g.wrapped == null && f.payload == null && g.payload == null then
        f.type
      else if f.wrapped != null && g.wrapped != null && wrapped != null then
        f.type wrapped
      else if f.payload != null && g.payload != null && payload != null then
        f.type payload
      else
        null;
    ...
    withCheck = type: check: type // { check = value: type.check value && check value; };
    ...
    any = lib.types.create {
      ...
      merge =
        ...
        let
          ...
          getType =
            value:
            if builtins.isAttrs value && lib.strings.validate.stringifiable value then
              "StringifiableAttrs"
            else
          ...
          merge =
            if commonType == "set" then
              mergeSet
            else if commonType == "list" then
              mergeList
            else if commonType == "StringifiableAttrs" then
              mergeStringifiableAttrs
            else if commonType == "lambda" then
              mergeLambda
            else
              lib.options.merge.equal;
        in
    ...
    ints =
      ...
      in
      {
        ...
        ##
        ## @type Attrs
        positive = lib.types.withCheck lib.types.int (value: value > 0) // {
          name = "IntPositive";
        ...
        ##
        ## @type Attrs
        unsigned = lib.types.withCheck lib.types.int (value: value >= 0) // {
          name = "IntUnsigned";
    ...
    numbers =
      ...
      in
      {
        ...
        ##
        ## @type Attrs
        positiveOrZero = lib.types.withCheck lib.types.int (value: value >= 0) // {
    ...
    strings = {
        ...
        lib.types.create {
          ...
          description =
            if separator == "" then
              ...
              "string concatenated with ${builtins.toJSON separator}";
          check = value: lib.types.string.check value;
          merge =
            location: definitions:
            builtins.concatStringsSep separator (lib.options.getDefinitionValues definitions);
          functor = lib.types.functor "strings.concat" // {
            payload = separator;
            merge = x: y: if x == y then x else null;
          };
        };
      ...
      line =
        ...
        lib.types.create {
          ...
          check = matcher.check;
          merge = location: definitions: lib.strings.removeSuffix "\n" (matcher.merge location definitions);
        };
    ...
    attrs = {
      ...
      any = lib.types.create {
        ...
        merge =
          location: definitions:
          builtins.foldl' (result: definition: result // definition.value) { } definitions;
      };
        ...
        lib.types.create {
          ...
          merge =
            ...
            let
              ...
              filtered = lib.attrs.filter (name: value: value ? value) (builtins.zipAttrsWith zipper normalized);
            in
    ...
    derivation = lib.types.create {
      ...
      merge =
        ...
        in
        if builtins.isPath merged || (builtins.isString merged && !(builtins.hasContext merged)) then
          lib.paths.into.drv merged
        else
          merged;
    };
    ...
    path = lib.types.create {
      ...
      check =
        value:
        lib.strings.validate.stringifiable value && builtins.substring 0 1 (builtins.toString value) == "/";
      merge = lib.options.merge.equal;
    };
    ...
    list = {
      ...
      required =
        type:
        lib.types.withCheck (lib.types.list.of type) (value: value != [ ])
        // {
    ...
    nullish =
      ...
      lib.types.create {
        ...
        merge =
          ...
          in
          if totalNulls == builtins.length definitions then
            null
          else if totalNulls != 0 then
            builtins.throw "The option `${identifier}` is defined as both null and not null in ${serializedFiles}"
          else
    ...
    submodule = modules: lib.types.submodules.of { modules = lib.lists.from.any modules; };
      ...
      of =
        ...
        let
          getModules = builtins.map (
            definition:
            if shorthand && builtins.isAttrs definition.value then
              let
        ...
        lib.types.create {
          inherit name;
          description = if description != null then description else freeform.description or name;
          fallback = {
            value = { };
          };
          ...
          children = lib.attrs.when (freeform != null) { inherit freeform; };
          functor = lib.types.functor "submodule" // {
            ...
            merge = x: y: {
              ...
              args =
                ...
                if intersection == { } then
                  ...
                  builtins.throw "A submodule option is declared multiple times with the same args: ${builtins.toString (builtins.attrNames intersection)}";

              description =
                if x.description == null then
                  y.description
                else if y.description == null then
                  x.description
                else if x.description == y.description then
                  x.description
                else
                  builtins.throw "A submodule description is declared multiple times with conflicting values";
            };
          };
        };
    };
    ...
    deferred = {
      ...
      ##
      ## @type Attrs
      default = lib.types.deferred.of { modules = [ ]; };
      ...
      of =
        ...
        lib.types.create {
          ...
          functor = lib.types.functor "deferred.of" // {
            ...
            merge = x: y: { modules = x.modules ++ y.modules; };
          };
        };
    };
      ...
      lib.types.create {
        ...
        description =
          if values == [ ] then
            ...
            "value ${serialize (builtins.elemAt values 0)} (singular enum)"
          else
            "one of ${lib.strings.concatMapSep ", " serialize values}";
        check = value: builtins.elem value values;
        merge = lib.options.merge.equal;
        functor = lib.types.functor "enum" // {
          payload = values;
    ...
    either =
      ...
      lib.types.create {
        ...
        merge =
          ...
          in
          if isLeft then
            left.merge location definitions
          else if isRight then
            right.merge location definitions
          else
            lib.options.merge.one location definitions;
        mergeType =
          f:
          let
          ...
          in
          if (f.name == name) && (mergedLeft != null) && (mergedRight != null) then
            functor.type mergedLeft mergedRight
          else
            null;
        children = {
          inherit left right;
    ...
    one =
      ...
      in
      if types == [ ] then
        builtins.throw "lib.types.one must be given at least one type"
      else
        builtins.foldl' lib.types.either first rest;
      ...
      if initial.getSubModules != null then
        ...
        lib.types.create {
          ...
          merge =
            location: definitions:
            let
              process = value: if initial.check value then transform value else value;
      ...
      entry =
        ...
        let
          submodule = lib.types.submodule (
            ...
            {
              options = {
                value = lib.options.create { type = type; };
                before = lib.options.create { type = lib.types.list.of lib.types.string; };
                after = lib.options.create { type = lib.types.list.of lib.types.string; };
              };
            }
          );
          normalize =
            ...
            let
              value =
                if definition ? priority then
                  lib.modules.order definition.priority definition.value
                else
                  definition.value;
            in
            if lib.dag.validate.entry definition.value then definition.value else lib.dag.entry.anywhere value;
        in
        lib.types.create {
          name = "DagEntry";
